...
DELETE FROM staging_test.staging_translations WHERE content_code IN ('code1-test', 'code2-test') AND language_code in ('lan', 'gua') AND namespace in ('backend');
...
DELETE FROM reporting_test.translations WHERE content_code IN ('code1-test', 'code2-test') AND language_code in ('lan', 'gua') AND namespace in ('backend');
